...
#
# This file is responsible for emptying an Amazon S3 bucket.
# References:
...
if [ "$#" -ne 8 ]; then
  echo "Must pass cfn_stack_prefix, app_stage, deploy_id, cfn_stack_suffix, bucket_cfn_export_name_suffix, supply_cfn_export_key, region, and aws_cli_profile_arg as parameters, exiting"
  exit 1
fi
...
if [ "$supply_cfn_export_key" == "null" ]; then 
  export cfn_export_key=$(./build-utils/infra/aws.strip-dashes.sh "${aws_cfn_prefix}-${app_stage}-${bucket_cfn_export_name_suffix}-${deploy_id}")
else 
  echo "supplied cfn export: $bucket_cfn_export_name_suffix"
  export cfn_export_key=$(echo $bucket_cfn_export_name_suffix)
fi
echo "lookup cfn export: $cfn_export_key"

bucket_name=$(aws cloudformation describe-stacks --stack-name "${aws_cfn_prefix}-${app_stage}-${cfn_stack_suffix}-${deploy_id}" $aws_cli_profile_arg --region "$region" --query 'Stacks[0].Outputs[?OutputKey==`'$cfn_export_key'`].OutputValue' --output text)
