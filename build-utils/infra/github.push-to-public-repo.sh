#!/usr/bin/env bash
# This file is responsible for pushing your private source to the public repository.
# Purge anything you want removed here!
# This file has some global side effects! It modifies global Git config.
# References:
...
cd ${tmp_dir}; mkdir -p mainframenzo.com; cd ${tmp_dir}/mainframenzo.com; git init; git remote add origin git@github.com:mainframenzo/mainframenzo.com.git
...
# Copy the contents of _this_ private repository to the temporary public repository.
rsync -av --progress $cwd/ ${tmp_dir}/mainframenzo.com/ --exclude .git --exclude node_modules --exclude dist.*

# Prepare the temporary version of this source code for public viewing.
cd ${tmp_dir}/mainframenzo.com; rm -rf node_modules; ls -al; just -f ./.justfiles/dev.just --working-directory . setup --skip-python-deps; npx node \
  --trace-uncaught \
  --enable-source-maps \
...
# FIXME Run something like this: https://github.com/secretlint/secretlint/tree/master

if [[ "${dry_run}" == "true" ]]; then
  # FIXME This oddly causes an error.
  #tree "${tmp_dir}/mainframenzo.com"
...
# The public dirs hold the largest files. Add each one as a separate commit so you can commit in batches.
