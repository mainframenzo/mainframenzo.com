...
export default class CICDPipelineIAMPolicies extends Constructs.Construct {
  ...
  readonly allowCdkBucketAccess: IAM.PolicyStatement;
  ...
  constructor(scope: Constructs.Construct, id: string, props: Props) {
    ...
    // You don't like using saved CDK context.json, but alternatively you can. See: 
    ...
    // FIXME You should be able to pass lambdas in since we're self-mutating now, use prefix perhaps otherwise.
    this.allowInvokeLambda = policies.allowInvokeLambdaFunction('*');

    // Needed to download source code manually because of how our CICD trigger is setup. 
    this.allowSourceCodeBucketAccess = policies.allowBucketReadAccess(this.props.cicdSourceBucket.bucketArn);

    // Needed to download our development container image used to build the frontend during CICD.
    // You just stored a Docker registry in the CDK bucket so you didn't need any more buckets.
    this.allowCdkBucketAccess = policies.allowBucketReadAccess(`arn:aws:s3:::cdk-${props.stage.getConfig().cdkQualifier}-assets-${props.stage.getConfig().accountID}-${props.stage.getConfig().region}`);
