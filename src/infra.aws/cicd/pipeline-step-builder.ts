  ...
  createStepFromCommands(stepName: string, buildCommands: BuildCommand[]): CodePipeline.StageProps {
    ...
    buildCommands.forEach(buildCommand => {
      if (!buildSteps.has(buildCommand.actionName)) {
        ...
        this.props.pipelineIAMPolicies.forEach(policy => buildCommand.policyStatements?.push(policy));
...
interface Props extends constants.CommonProps {
  ...
  readonly pipelineIAMPolicies: IAM.PolicyStatement[];
  readonly envs: BuildEnvs;
}
...
export interface BuildCommand {
  ...
  readonly env?: string; // Only set in initial command def. // FIXME lost typing on this.
