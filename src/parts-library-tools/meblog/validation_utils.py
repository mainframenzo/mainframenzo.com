# This file is responsible for providing meblog validation utilities.
# References:
...
class STLUtils:
  ...
  def calculateVolume(self, unit, material_mass):
    totalVolume = sum(
        self.signedVolumeOfTriangle(p1, p2, p3) for p1, p2, p3 in self.triangles
    ) / 1000
...
def main():
  ...
  parser.add_argument(
      'calculation',
      choices=['volume', 'area'],
      help='Choose between calculating volume or surface area'
  )
  parser.add_argument(
      '--unit',
      choices=['cm', 'inch'],
      default='cm',
      help='Unit for the volume calculation (default: cm)'
  )
  parser.add_argument(
      '--material',
      type=int,
      choices=range(1, 21),
      default=2,
      help='Material ID for mass calculation'
  )
  parser.add_argument(
      '--filetype',
      choices=['stl', 'nii', 'dcm'],
      default='stl',
      help='Type of the input file: stl, nii, dcm'
  )
