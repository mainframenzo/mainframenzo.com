...
def from_assembly_file(file_path: str):
  ...
  # You use the parent directory of the assembly as the root Python module directory.
...
def import_stl_jdegenstein(file_name: PathLike | str | bytes, model_unit: Unit = Unit.MM) -> Face:
  ...
  if model_unit == Unit.MM:
    ...
    try:
      ...
      raise ValueError(f"model_scale must one a valid unit: {Unit._member_names_}")
...
def _process_leader(entity):
  ...
  edges = [Line(start=vertices[i], end=vertices[i + 1]) for i in range(len(vertices) - 1)]

  # Calculate arrow size based on leader length or use a default
  leader_length = sum(
      Line(start=vertices[i], end=vertices[i + 1]).length() for i in range(len(vertices) - 1)
  )
  arrow_size = (
...
def _process_text(entity):
  ...
  content = (entity.dxf.text if entity.dxftype() == "TEXT" else entity.text)  # Text content
  height = (
      entity.dxf.height if entity.dxftype() == "TEXT" else entity.dxf.char_height
  )  # Text height
  rotation = (
      entity.dxf.rotation if entity.dxftype() == "TEXT" else entity.dxf.get("rotation", 0)
  )  # Rotation angle
