...
def create_spot_light():
  ...
  spot_light = bpy.data.objects.new(
      name="spot_light_object_{}".format(str(uuid.uuid4())), object_data=spot_light_data
  )
  spot_light.location = (0, 2, 2)  # Adjust the location as needed.
  spot_light.rotation_euler = (
      4.71, 0, 0
  )  # 4.71,0,0 shine straight # (-0.8, 0, 0) # Adjust the rotation as needed.
...
def angular_distance(phi1, theta1, phi2, theta2):
  ...
  cos_angle = math.sin(phi1) * math.sin(phi2) + math.cos(phi1) * math.cos(phi2) * math.cos(
      theta1 - theta2
  )
...
def sample_hemisphere_around_object(obj):
  ...
  bounding_box_size = Vector(
      (
          max(vertex[0] for vertex in bounding_box_vertices) -
          min(vertex[0] for vertex in bounding_box_vertices),
          max(vertex[1] for vertex in bounding_box_vertices) -
          min(vertex[1] for vertex in bounding_box_vertices),
          max(vertex[2] for vertex in bounding_box_vertices) -
          min(vertex[2] for vertex in bounding_box_vertices)
      )
  )
