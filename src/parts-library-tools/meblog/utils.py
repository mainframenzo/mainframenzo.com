...
def _parse_parametric_object(
    assembly, object: ParametricObject, name: str, material: Union[str, None]
  ...
  # Do you really need to support per-child assemblies? Could you just get CadQuery to flatten into a shape for us?
  if isinstance(object, cadquery.Assembly):
    ...
    return ParametricObjectNode(name=name, material=material, children=children)
  ...
  if isinstance(object, ParametricShape):
    ...
    # FIXME `object.val().wrapped` is not guaranteed to be `Shape`.
  ...
  return ParametricObjectNode(name=name, material=material, vertices=mapped_vertices, faces=faces)
