...
# FIXME This import causes trouble. meblog.types requires cadquery + build123d dependencies, and it's making life hell
#  when importing blender_utils to render .stl files to .png files (no cadquery or build123d needed).
# Just duplicate the type for now.
...
def enable_cuda_devices():
  ...
  # If you have CUDA/OPENCL devices, enable only them, otherwise enable
  ...
  if not accelerated:
    raise Exception("failed to enable cuda device")


# Framework laptop.
def enable_hip_devices():
  cycles_prefs = bpy.context.preferences.addons['cycles'].preferences

  # Force the backend to HIP (AMD's standard for Blender 3.0+).
  cycles_prefs.compute_device_type = 'HIP'

  # Refresh devices to ensure the AMD GPU is detected.
  cycles_prefs.get_devices()

  # Enable all available HIP devices (GPU) and disable others (CPU).
