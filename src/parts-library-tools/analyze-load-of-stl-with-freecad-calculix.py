...
# can you determine contraints automatically?
...
# FIXME pass fea info from build123d to stl to freecad?
...
def main():
  try:
    ...
    print(
        'FreeCAD library not found. Please check the FREECADPATH variable in the import script is correct'
    )
  ...
  try:
    ...
    # FIXME relative path
    doc.saveAs(
        '/Users/$USER/dev/mainframenzo.com-private/src/frontend/public/parts-libraries/posts/globe_trotter_suitcase/assembly.FCStd'
    )
...
def import_assembly(doc):
  ...
  # FIXME relative path
  mesh.read(
      '/Users/$USER/dev/mainframenzo.com-private/src/frontend/public/parts-libraries/posts/globe_trotter_suitcase/assembly.stl'
  )
...
def run_analysis(doc, analysis):
  ...
  if not message:
    ...
    # if you would write the inp file anyway, you need to again set it manually
