...
#   sys.path.append('/opt/homebrew/anaconda3/envs/meblog-stl-to-photorealistic-pngs/lib/python311.zip')
#   sys.path.append('/opt/homebrew/anaconda3/envs/meblog-stl-to-photorealistic-pngs/lib/python3.11')
#   sys.path.append('/opt/homebrew/anaconda3/envs/meblog-stl-to-photorealistic-pngs/lib/python3.11/lib-dynload')
#   sys.path.append('/opt/homebrew/anaconda3/envs/meblog-stl-to-photorealistic-pngs/lib/python3.11/site-packages')

# if platform.system() == 'Linux':
#   sys.path.append('/usr/local/lib')
#   sys.path.append('/opt/app/meblog')
#   sys.path.append('/opt/conda/envs/meblog-stl-to-photorealistic-pngs/lib/python311.zip')
#   sys.path.append('/opt/conda/envs/meblog-stl-to-photorealistic-pngs/lib/python3.11')
#   sys.path.append('/opt/conda/envs/meblog-stl-to-photorealistic-pngs/lib/python3.11/lib-dynload')
#   sys.path.append('/opt/conda/envs/meblog-stl-to-photorealistic-pngs/lib/python3.11/site-packages')

# Add "meblog" Python module.
modules_dir = os.path.dirname(os.path.realpath(__file__))
sys.path.append(modules_dir)

from meblog.blender_utils import init_scene, enable_cuda_devices, enable_hip_devices
...
def _parse_args():
  ...
  fast_render = json.loads(argv[2])
  if len(argv) == 4:
    stl_metadata_file = argv[
        3
    ]  # Optional. The .stl.json file containing metadata used in rendering (.stl format no good for this).

  print('blender args', argv)
  print('input_file', input_file)
  print('output_directory', output_directory)
  print('fast_render', fast_render)
...
def _configure_output_settings():
  ...
  if fast_render:
    print('fast render')
    # Low sample count, minimal bounces, denoiser to compensate.
    # This is not high enough fidelity for prod, but works in a pinch.
    ...
    bpy.context.scene.cycles.filter_width = 1.5  # Wider filter reduces noise at the cost of sharpness.

    bpy.context.scene.cycles.use_adaptive_sampling = True
    bpy.context.scene.cycles.adaptive_threshold = 0.1  # Stops sooner, more noise.
    bpy.context.scene.cycles.adaptive_min_samples = 8

    # Suppress fireflies so denoiser has less work.
    bpy.context.scene.cycles.sample_clamp_direct = 3.0
    bpy.context.scene.cycles.sample_clamp_indirect = 1.0

    if bpy.context.scene.cycles.device == 'GPU':
      # Large tiles are faster on GPU.
      bpy.context.scene.cycles.tile_x = 256
      bpy.context.scene.cycles.tile_y = 256

      bpy.context.scene.render.use_persistent_data = True  # Keep scene on GPU.
    else:
      bpy.context.scene.cycles.tile_x = 32
      bpy.context.scene.cycles.tile_y = 32
  else:
  ...
  try:
    ...
    print('failed to enable cuda devices')
  ...
  try:
    ...
    print('failed to enable hip devices')
...
def _configure_depth_output(active_view_layer, nodes, links, render_layers):
  ...
  map.offset = [
      -1
  ]  # Only objects that are at least 1 blender unit away from the camera are recognoized for the depth. Every closer object is ignored.
  ...
  map.min = [
      0
  ]  # If you want another distance-delay in the range where the depth map is actually neither black nor white.
...
def _configure_scene_lighting(imported_stl_object):
  ...
  bpy.context.scene.world.color = (
      0, 0, 0
  )  # No ambient lightning, adjust the RGB values as needed.
  ...
  while angular_distance(phi, theta, phi3,
                         theta3) < 30 or angular_distance(phi2, theta2, phi3, theta3) < 30:
...
def _configure_output_camera():
  ...
  cam.location = (
      0, 2.3, 0
  )  # This controls the distance - you later only rotate this camera, but the distance stays the same.
  cam.data.lens = 35
  ...
  bpy.context.scene.collection.objects.link(
      cam_empty
  )  # The cam_empty camera becomes visible as it is added to this scenes collection.
...
def _render(camera, depth_file_output, normal_file_output, albedo_file_output):
  ...
  for view_number in range(number_of_views):
    ...
    # if _render_outputs_exist(render_file_prefix):
    #   print(f"skipping view {view_number}, outputs already exist: {render_file_prefix}")

    #   continue

    _render_pngs_at_view(
        camera, view_number, depth_file_output, normal_file_output, albedo_file_output
    )

    bpy.context.view_layer.update()  # FIXME May not need this.


# def _render_outputs_exist(render_file_prefix):
#   patterns = [
#     f"{render_file_prefix}.png",
#     f"{render_file_prefix}-depth*.png",
#     f"{render_file_prefix}-albedo*.png",
#     f"{render_file_prefix}-normal*.png",
#   ]

#   return all(glob.glob(p) for p in patterns)


def _render_pngs_at_view(
    camera, view_number, depth_file_output, normal_file_output, albedo_file_output
):
