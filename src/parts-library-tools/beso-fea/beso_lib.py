...
def import_FI_int_pt(reference_value, file_nameW, domains, criteria, domain_FI, file_name, elm_states,
    ...
    try:
        ...
        write_to_log(file_name, "\nERROR: " + msg + "\n")
        assert False, msg
    last_time = "initial"  # FIXME solve how to read a new step which differs in time
    step_number = -1
    ...
    for line in f:
        ...
        if line.replace(" ", "") == "\n":
            ...
            if line.split()[-4] in map(lambda x: x.upper(), domains_from_config):  # FIXME upper already on user input
                read_stresses = 2
