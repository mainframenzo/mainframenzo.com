...
export default class SwipeEvent {
  ...
  getSwipeDirection(): TSwipeDirection | undefined {
    if (!this.startEvent.changedTouches || !this.endEvent?.changedTouches) { 
      console.warn('can not determine wipe direction: start or end touch info missing'); 
    ...
    if (!start || !end) { 
      console.warn('can not determine wipe direction: start or end pixels missing'); 
