#!/usr/bin/env bash
# This file is responsible for providing bash functions which correlate to our monitor stats.
# It was pulled from https://github.com/andchir/linux-dash2/blob/master/app/server/linux_json_api.sh, 
#  and some unused functions were removed.
ECHO=$(type -P echo)
SED=$(type -P sed)
GREP=$(type -P grep)
TR=$(type -P tr)
...
cpu_utilization() {
  ...
  while [[ iteration -lt 2 ]]; do
    ...
    # Calculate the CPU usage since you last checked.
